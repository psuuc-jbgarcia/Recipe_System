...
if (
    ...
    if ($isValid) {
        ...
        if (
            ...
            $confirmpassword == "" || $firstname == "" || $lastname == "" || $sex == ""
        ) {
            $response = [
                'title' => '',
    ...
    if ($isValid) {
        ...
        if ($password === $confirmpassword) {
            ...
            if ($username == $adminuser) {
                ...
                $insertQuery = "INSERT INTO admin_account (username, email, password, firstname, lastname, sex) VALUES (?, ?, ?, ?, ?, ?)";
            } else {
                // Insert into account table
                $insertQuery = "INSERT INTO account (username, email, password, firstname, lastname, sex) VALUES (?, ?, ?, ?, ?, ?)";
            }

            $stmt = $conn->prepare($insertQuery);
            $stmt->bind_param("ssssss", $username, $email, $hashedPassword, $firstname, $lastname, $sex);
